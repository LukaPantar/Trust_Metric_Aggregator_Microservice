...
def main():
    ...
    rate = query_prometheus(prometheus_url, PromqlFunction.RATE, MetricUrl.THROUGHPUT, MetricLabels.NETWORK_DEVICE, 1)
    if rate:
        print(f"Rate of throughput ({MetricUrl.THROUGHPUT}) for {MetricLabels.NETWORK_DEVICE}: {rate:.2f} bytes/second")
    else:
